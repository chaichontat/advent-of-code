...
impl Universe {
    ...
    fn set_units(&mut self, initiative: usize, units: u32) {
        if units == 0 {
            self.alive[self.groups[initiative].team as usize] &= !(1 << initiative);
        }
        self.groups.get_mut(initiative).unwrap().set_units(units);
        self.rearrange_select_order(initiative);
        debug_assert!(self.is_sound());
    }

    fn rearrange_select_order(&mut self, changed: usize) {
        let defender = self.groups[changed];
        let mut i = self
            .select_ord
            .iter()
        ...
        while i + 1 < N_UNITS && defender.ep < self.groups[self.select_ord[i + 1]].ep {
...
fn fn_select_ord(groups: &[Group], i: &usize) -> (Reverse<u32>, u32) {
...
fn miniparse(groups: &mut [Group], lines: &[String], team: Team) -> Option<bool> {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        groups[i] = Group {
            ...
            dp,
            ep: n * dp,
            initiative: N_UNITS as u32 - initiative,
            weak,
            immune,
            target: None,
            team,
...
fn run(mut uni: Universe) -> Option<Universe> {
    while uni.alive[0] > 0 && uni.alive[1] > 0 {
        ...
        while attacking != 0 {
            ...
            let temp_defender = uni.groups[attacker.target.unwrap()];
            ...
            let defender = &uni.groups[attacker.target.unwrap()];
            if defender.units == 0 {
