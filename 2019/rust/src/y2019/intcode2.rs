...
macro_rules! from_mode_opcode {
    ( $( $t:ident)* ) => {
        $(
            impl From<$t> for OpCode {
                fn from(op: $t) -> Self {
                    match op {
                        1 => OpCode::Add,
                        2 => OpCode::Mul,
...
from_mode_opcode!(u8 i8 u16 i16 u32 i32 u64 i64 usize isize);

#[derive(Debug, Clone)]
pub struct IntCode<T: PrimInt> {
    pub mem:    Vec<T>,
    pub ptr:    usize,
    pub rel:    T,
    pub done:   bool,
        ...
        $(
            ...
            impl IntCode<$t> {
                ...
                pub fn run_wait_input(&mut self) {
                    while !self.done {
                        let op: OpCode = OpCode::from(self.mem[self.ptr] % 100);
                ...
                fn op_lteq(&mut self, op: OpCode, cmd: ($t, $t), p: usize) {
                ...
                fn set(&mut self, to_set: $t, addr: usize) {
                ...
                fn fetch_ins(&self, ptr: usize) -> ($t, [$t; 3]) {
                ...
                fn fetch_data(&self, mode: &Mode, addr: $t) -> $t {
                    match mode {
                ...
                fn fetch_write(&self, mode: &Mode, addr: $t) -> usize {
                    match mode {
                ...
                fn parse_ins(&self, mut ins: $t) -> (OpCode, [Mode; 3]) {
                    let mut modes = [Mode::Pos, Mode::Pos, Mode::Pos];
                    let op: OpCode = OpCode::from(ins % 100);
                    ins /= 100;
                    for mode in &mut modes {
                        *mode = Mode::from(ins % 10);
                        ins /= 10;
                    }
                    (op, modes)
                }
...
intcode!(u8 i8 u16 i16 u32 i32 u64 i64 usize isize);

#[cfg(test)]
mod tests {
    use super::*;

    fn prep<T: PrimInt>(mem: &[T]) -> Vec<T> {
    ...
    fn test_out(mem: &[isize], ins: &[isize], outs: &[isize]) {
        let mut ic = IntCode::from(mem);
        ic.input.append(&mut VecDeque::from(ins.to_vec()));
        ic.run();
